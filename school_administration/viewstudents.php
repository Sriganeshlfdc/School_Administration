...
$filter_stream = filter_input(INPUT_GET, 'stream', FILTER_SANITIZE_STRING) ?? ''; // NEW
...
$filter_gender = filter_input(INPUT_GET, 'gender', FILTER_SANITIZE_STRING) ?? ''; // NEW
...
if ($conn) {
    $sql = "
        SELECT 
            ...
            P.guardian_name, P.guardian_relation, P.guardian_contact,
    ...
    if (!empty($search_id)) {
        ...
        if (!empty($filter_stream)) { $sql .= " AND E.Stream = ?"; $types .= 's'; $params[] = $filter_stream; } // NEW
        ...
        if (!empty($filter_gender)) { $sql .= " AND S.Gender = ?"; $types .= 's'; $params[] = $filter_gender; } // NEW
    }

    $sql .= " ORDER BY S.StudentID ASC LIMIT 200";
    ...
    <table class="data-table">
        ...
        <tbody>
            <?php if (empty($results)): ?>
                <tr><td colspan="7" style="text-align: center;">No students found matching the criteria.</td></tr>
            <?php else: ?>
                <?php foreach ($results as $student): ?>
                    <tr>
                        ...
                        <td>
                            <a href="studentprofile.php?id=<?php echo $student['StudentID']; ?>">
                                <button class="btn-action view-btn profile-trigger-btn" data-id="<?php echo $student['StudentID']; ?>"><i class="fa fa-user"></i> Profile</button>
                            </a>
                        </td>
                    </tr>
                <?php endforeach; ?>
