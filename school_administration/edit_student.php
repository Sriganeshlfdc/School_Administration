...
try {
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($file['size'] <= $MAX_FILE_SIZE && allowed_file($filename, $ALLOWED_EXTENSIONS)) {
            ...
            if (!is_dir($STUDENT_DIR) && !mkdir($STUDENT_DIR, 0755, true)) throw new Exception("Failed to create dir");
            ...
            if (move_uploaded_file($file['tmp_name'], $file_path)) {
                ...
                if ($old_photo_path) {
                    $old_abs_path = realpath($UPLOAD_DIR_BASE . str_replace($UPLOAD_FOLDER_REL, '', $old_photo_path));
                    if ($old_abs_path && strpos($old_abs_path, realpath($UPLOAD_DIR_BASE)) === 0 && file_exists($old_abs_path)) {
                        unlink($old_abs_path);
                    }
                }
            }
    ...
    function build_dynamic_update($table, $id_col, $id_val, $field_map, $conn) {
        ...
        foreach ($field_map as $post_key => $db_col) {
            if (array_key_exists($post_key, $_POST)) {
                ...
                // FORMAT ACADEMIC YEAR IF EDITED
    ...
    // 3. Enrollment Map (Updated AcademicYear)
    $enrollment_map = [
        'Level' => 'Level',
        'Class' => 'Class',
        'Stream' => 'Stream',
        ...
        'AcademicYear' => 'AcademicYear' // Mapped from RegistrationYear input via JS or handled above
    ];
